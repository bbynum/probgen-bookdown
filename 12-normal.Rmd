...
Taylor is studying nursing and knows that SBP for women is normally distributed with a mean of 115 and a standard deviation of 14. Find the $95\%$ middle range for SBP for women.

### Exercise - Swimming in the Ocean
Emily likes swimming in the ocean and notices that high tides vary and thinks it may be a normal distributed. If this is so and the average high tide is 0.6 meters with a standard deviation of 0.2 meters what is the probability a high tide will be between 0.8 and 1.0 meters.
...
Sketches by old master sell for about $\$500$ with a standard deviation of $\$125$. Marijke wants to find the middle range, that is, the lower and upper cutoffs within which $50\%$ of old master sketches will sell. 
...
Jasmine likes sloths. During the sloth Olympics, the average time for the three meter dash is 46 seconds, normally distributed with a standard deviation of 5 seconds. For a sloth chosen at random, what is the probability their three meter dash time will be greater than 50 seconds?
...
```{r nice-fig-123, fig.cap='Sample Space for Game Two', out.width='30%', fig.asp=.75, fig.align='center', echo=FALSE}
...
```{r nice-fig-124, fig.cap='The Pythagorean Theorem of Standard Deviations', out.width='30%', fig.asp=.75, fig.align='center', echo=FALSE}
...
Recall, Friday tips are normal and Saturday tips are normal with $F \sim N(\mu = 120, \sigma = 30)$ and $S \sim N(\mu = 90, \sigma = 20)$. Before examining the theory, let's run a simulation to get an idea of what to expect.
...
What really happens when adding independent normal random variables is that the shape is normal, the mean is the sum of the individual means but the standard deviation is the square root of the sum of the squares of the individual standard deviations, a result akin to our familiar Pythagorean Theorem. In summary, given independent normal distributions X and Y with $X \sim N(\mu_{X}, \sigma_{X})$ and $Y \sim N(\mu_{Y}, \sigma_{Y})$ we know $X+Y \sim N(\mu_{X}+\mu_{Y}, \sqrt{\sigma_{X}^{2}+\sigma_{Y}^{2}})$.
...
```{r nice-fig-1212, fig.cap='Zombie Apocalypse Characters', out.width='30%', fig.asp=.75, fig.align='center', echo=FALSE}
...
```{r nice-fig-1213, fig.cap='Zombie Apocalypse Data Table', out.width='30%', fig.asp=.75, fig.align='center', echo=FALSE}
...
```{r nice-fig-1214, fig.cap='Dungeons and Dragons Dice', out.width='30%', fig.asp=.75, fig.align='center', echo=FALSE}
