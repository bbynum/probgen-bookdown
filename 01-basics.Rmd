...
**gene**: the DNA that controls a trait (e.g. hemoglobin beta gene, aka locus)  

**variant**: one of several versions of a gene (e.g. HbS variant in hemoglobin beta that can cause Sickle Cell Anemia, aka allele)  
...
intro to genetics
humans are diploid - 2 copies of every gene (can be same variant or different)
meiosis making gametes
fertilization

2 chromosomes, one from mom and one from dad. just like H and T.
you can think of each parent like a coin for each gene, and each gene-coin has 2 sides - one for each copy of the gene.

gene and variant "notation" (don't call it that)
...
```{r}
# gametes for parents
parent1_gametes <- c('A1','A1')
parent2_gametes <- c('A2','A2')

# list of 1000 gametes from each parent
parent1 <- sample(parent1_gametes, 1000, replace = TRUE)
parent2 <- sample(parent2_gametes, 1000, replace = TRUE)

# put alleles together to make 1000 offspring
cross_x1 <- paste(parent1, parent2, sep="/")
offspring1 <- data.frame(table(cross_x1))

# table
knitr::kable(offspring1, caption = 'A1/A2 first cross simulation', booktabs = TRUE)
...
```{r}
# simulation of a simple genetic cross

# 
parent3_gametes <- c('A1','A2')
parent4_gametes <- c('A1','A2')

# 
parent3 <- sample(parent3_gametes, 1000, replace = TRUE)
parent4 <- sample(parent4_gametes, 1000, replace = TRUE)

#
cross_x2 <- paste(parent3, parent4, sep="/")

offspring2 <- data.frame(table(cross_x2))
offspring2

# makes a bar graph of the frequency of genotypes
# offspring is the data frame with results, aes() tells ggplot what to use for X and Y
# geom_bar() makes the bars, and stat="identity" uses the Y value for the bar height
...
```{r}
# 
parent1_gametes <- c('B','B')
parent2_gametes <- c('b','b')
...
```{r}
# long form simulation of a simple genetic cross

# 
parent3_gametes <- c('B','b')
parent4_gametes <- c('B','b')
